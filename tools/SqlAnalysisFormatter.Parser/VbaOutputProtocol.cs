...
{
    private const string PlanHeader = "SAF_OUTPUT_PLAN\t6";
    ...
    public static string SerializePlan(OutputSheetPlan plan)
    {
        ...
        lines.AddRange(plan.TableNameReferences
            .OrderBy(item => item.Row)
            .ThenBy(item => item.Column)
            .ThenByDescending(item => item.SourceValue.Length)
            .ThenBy(item => item.SourceValue, StringComparer.Ordinal)
            .Select(item =>
                $"N\t{item.Row}\t{item.Column}\t{Escape(item.SourceValue)}\t" +
                $"{Escape(item.PhysicalTableId)}\t{Escape(item.ReplacementSuffix)}"));
        ...
        lines.AddRange(plan.TransformedQueryLines
            .OrderBy(item => item.QueryLine)
            .Select(item => $"R\t{item.QueryLine}\t{Escape(item.Value)}"));
        lines.AddRange(plan.ReplacementValues
            .OrderBy(item => item.QueryLine)
            .ThenBy(item => item.Order)
            .Select(item => $"V\t{item.QueryLine}\t{item.Order}\t{Escape(item.Value)}"));
        lines.AddRange(plan.InputTableIds.Select(tableId => $"T\tINPUT\t{Escape(tableId)}"));
        lines.AddRange(plan.OutputTableIds.Select(tableId => $"T\tOUTPUT\t{Escape(tableId)}"));
