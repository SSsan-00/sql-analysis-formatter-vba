...
INSERT SELECTやUPDATE、DELETEの中にSELECT処理がある場合は、内側のSELECT表を先、最終的な`＜データ移送表＞`を後に出力します。
...
`run-output-golden-tests.ps1`は本体モジュールと書式回帰テスト補助を取り込み、登録済み64ケースを比較します。
保存済みの `SqlAnalysisFormatter.xlsm` にはテストモジュールを残しません。

CRUDテストケースの内容は`tests/CRUD_TEST_CASES.md`、登録済み64出力ケースは`tests/OutputReportCases.json`と`tests/SqlAnalysisFormatter.OutputExpectations.xlsx`にまとめています。ユーザーレビュー待ちの6ケースは[暫定実装ケース](docs/PROVISIONAL_OUTPUT_CASES.md)で確認できます。
