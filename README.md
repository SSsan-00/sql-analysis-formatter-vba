...
`テーブル一覧`はA～C列に`テーブルID`、`テーブル名称`、`番号`を登録するマスターです。テーブルIDは`users`のようなスキーマ名・別名を除いた物理名で照合し、大文字小文字と角括弧の違いは無視します。同じIDが複数ある場合は警告を表示して処理を続け、最初の登録を使用します。

`アウトプット②`は、A1から`＜入力情報＞`、4列空けたBA1から`＜出力情報＞`を表示します。FROM/JOINなどで読み取る実テーブルを入力情報、SELECT INTO・INSERT・UPDATE・DELETEと`OUTPUT ... INTO`の書込先を出力情報として、SQL内の初出順かつ重複なしで並べます。INSERT VALUESは入力情報なしとして扱います。CTE名、派生テーブル別名、`SQ1`などは出力せず、その内部で参照する実テーブルを対象にします。`テーブル一覧`に一致しない物理名は出力しません。

両ブロックはNo、テーブルID、テーブル名称、番号の4項目で構成し、Noだけを2列結合して中央揃えにします。ほかの項目は結合せず左揃えにし、項目単位の外枠と行罫線を表示します。ヘッダーは`アウトプット①`と同じ背景色です。フォールバック時はタイトルとヘッダーだけを表示します。
