...
たとえば`a tb1`、`b tb1`、物理テーブル`tb2`をUNIONした場合、`tb2`は使用済みなので`b`の表示別名は`tb3`になります。変換定義の所属テーブルIDを`a`、`b`、`tb2`、所属テーブル和名をそれぞれ`テーブルA`、`テーブルB`、`テーブルC`として登録すると、参照テーブルはSQL順に`テーブルA[tb1]、テーブルB[tb3]、テーブルC[tb2]`となります。2番目の名称検索には人工別名`tb3`ではなく物理ID`b`を使用します。`b`の変換定義がない場合も、`テーブル一覧`のID`b`に名称があれば同じ表示へ補完します。人工別名`tb3`の変換定義は名称検索に使用しません。
...
`アウトプット①`では、SELECT INTOやINSERTの移送先を含め、対応済みの物理テーブルを和名解決できない場合も`(和名未取得)[users]`のように物理IDを残します。このIDが`アウトプット②`へ実際に表示する入力・出力情報のテーブルIDと完全一致する場合は、`テーブル一覧`のテーブル名称だけを使って`ユーザー`のように補完します。IDは前後空白、外側の角括弧、大文字小文字を既存の照合規則で正規化します。変換定義ですでに取得済みの名称、`users`と一致しない`(和名未取得)[tb1]`、名称が空欄または未取得の明細は変更しません。
