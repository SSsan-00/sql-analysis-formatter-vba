...
`アウトプット①`の参照テーブル行に`(和名未取得)[users]`があり、`アウトプット②`へ実際に表示する入力・出力情報のテーブルIDが`users`と完全一致する場合は、`テーブル一覧`のテーブル名称だけを使って`ユーザー`のように補完します。IDは前後空白、外側の角括弧、大文字小文字を既存の照合規則で正規化します。変換定義ですでに取得済みの名称、`users`と一致しない`(和名未取得)[tb1]`、名称が空欄または未取得の明細、IDを持たない単独の`(和名未取得)`は変更しません。
