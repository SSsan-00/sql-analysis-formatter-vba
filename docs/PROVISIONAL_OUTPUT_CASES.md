...
2026-07-18時点で、登録済み期待値のうち3ケースは最終レビュー未実施となる。SEL-079はCASE結果に明示エイリアスがあるレビュー専用ケースとして入力だけを用意し、登録済み期待値へは追加していない。
...
| SEL-079 | エイリアス付きの複数CASE取得項目 | `paid_amount`と`refund_amount`を取得項目名に使い、人工的な`CASE結果`名を出さずに各分岐を表示する（レビュー待ち） |

SEL-073からSEL-075のSQLはA5:SQL Mk-2 2.21.2の`Ctrl+Q`で実整形している。SEL-079は既存のCASE整形規則に合わせたレビュー用入力として`tests/ManualOutputCases.json`へ保存している。
...
```powershell
powershell -ExecutionPolicy Bypass -File tools/Set-ManualOutputCase.ps1 -CaseId SEL-079
```
