...
- 複合SELECTの分岐間で同じSQL別名が異なる物理テーブルを指す場合は、最初の別名を維持し、後続の束縛へ複合クエリ内で未使用の`tbN`を表示専用で割り当てる。列修飾子、条件、結合、CASE、INSERT UNIONの移送パターンに加え、SQL解析B列の別名宣言・束縛参照とC列の変換内容も同じ表示別名へ統一する。A列の元SQL、文字列、コメント、内側の同名別名スコープ、列エイリアス、物理テーブル利用情報は変更しない。採番対象やJOIN相手の和名が変換定義で未取得でも、parserが表示位置と物理IDの対応を一意に特定し、Output②の物理IDとテーブル一覧の名称が一致すれば参照行とJOIN見出しを`和名[表示別名]`へ補完する。対応が曖昧な表示、人工別名、条件文字列は名称検索・置換の対象にしない。
- UNION分岐の通常JOIN、CROSS JOIN、APPLY、括弧付きJOINは参照テーブルと別名採番へ含める。物理テーブルIDに一意な和名定義がある場合は物理IDを優先し、同一分岐内の重複別名は推測変換せずフォールバックする。UNION全体のORDER BY/OFFSETは分岐外へ一度だけ出力する。
