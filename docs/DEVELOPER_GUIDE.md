...
描画計画プロトコルは`SAF_OUTPUT_PLAN 5`です。従来の`C`、`S`、`Q`、`F`、`T`に加え、`R`レコードでB列用の変換後SQLを論理行単位に、`V`レコードでC列以降へ出す最終変換値を論理行・出現順単位に返します。VBAは全`R`行の件数と重複、全`V`行のキーと順序を含む計画全体をメモリ上で検証してからB/C列を更新します。旧version 1～4では従来のB列と`Q`によるC列補完を維持し、parser未検出、実行失敗、不正なv5計画ではparser実行前に作成した和名変換結果を残します。
A列`-`のB列テーブル和名は、FROM内の実テーブルが1件かつ有効な和名が1種類の場合だけ参照テーブルのフォールバックへ使用します。複数テーブルまたは複数和名では推測しません。

複合クエリは1つの出力フレームへ統合するため、分岐をまたいで同じSQL別名が異なる物理テーブルへ束縛される場合だけ表示別名を採番します。採番計画はASTの物理テーブル束縛とスコープを保持し、文字列置換ではなく別名宣言と列修飾子のAST位置へ適用します。B列用SQLは未修飾列補完前のparser用SQLへ採番置換だけを適用して作り、C列用値はparser専用フィールドIDの各出現をAST位置へ対応付けて作ります。これにより1セル内にUNIONの両分岐がある場合も`tb1.和名`と`tb2.和名`を共存させます。文字列リテラル、コメント、引用識別子の引用形式、内側で同名別名を再定義したスコープは保持します。採番した表示別名は和名検索のキーにせず、元のSQL別名または物理テーブルIDで解決します。INSERT UNIONのSELECT表と移送パターンは同じ採番計画を共有し、テーブル利用情報は表示別名へ変換せず物理IDのまま保持します。
