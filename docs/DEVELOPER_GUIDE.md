...
描画計画プロトコルは`SAF_OUTPUT_PLAN 6`です。従来の`C`、`S`、`Q`、`F`、`T`に加え、`R`レコードでB列用の変換後SQLを論理行単位に、`V`レコードでC列以降へ出す最終変換値を論理行・出現順単位に返します。version 6の`N`レコードは、テーブル一覧から名称補完できるセル行・列、元表示、物理ID、表示接尾辞を返します。VBAは`N`のセル範囲、重複、元表示との一致を含む計画全体をメモリ上で検証してから、Output②へ実際に出す物理IDの有効名だけを適用します。旧version 1～4では従来のB列と`Q`によるC列補完、version 5ではB/C列の最終値と従来の参照行補完を維持します。parser未検出、実行失敗、不正な計画ではparser実行前に作成した和名変換結果を残します。
A列`-`のB列テーブル和名は、FROM内の実テーブルが1件かつ有効な和名が1種類の場合だけ参照テーブルのフォールバックへ使用します。複数テーブルまたは複数和名では推測しません。

複合クエリは1つの出力フレームへ統合するため、分岐をまたいで同じSQL別名が異なる物理テーブルへ束縛される場合だけ表示別名を採番します。採番計画はASTの物理テーブル束縛とスコープを保持し、文字列置換ではなく別名宣言と列修飾子のAST位置へ適用します。B列用SQLは未修飾列補完前のparser用SQLへ採番置換だけを適用して作り、C列用値はparser専用フィールドIDの各出現をAST位置へ対応付けて作ります。これにより1セル内にUNIONの両分岐がある場合も`tb1.和名`と`tb2.和名`を共存させます。文字列リテラル、コメント、引用識別子の引用形式、内側で同名別名を再定義したスコープは保持します。FROM句の通常JOIN、CROSS JOIN、APPLY、括弧付きJOINは同じTableReference走査で扱います。物理テーブルIDに一意な和名定義がある場合は元のSQL別名定義より物理IDを優先し、同一分岐内で別名が重複して参照先を決められない場合はフォールバックします。採番した表示別名は和名検索のキーにせず、元のSQL別名または物理テーブルIDで解決します。UNION全体のORDER BY/OFFSETは分岐とは別の末尾セクションへ一度だけ出力します。変換定義で名称を取得できない表示は、参照行と列Qのparser生成JOIN見出しに限り、`N`レコードでセル・元表示・物理ID・表示接尾辞をVBAへ渡します。同じ元表示が複数の物理IDへ対応する場合は`N`を生成せず、推測補完を禁止します。VBAもOutput②へ実際に出す物理IDとテーブル一覧の有効名が一致した場合だけ補完するため、JOIN条件、文字列、人工別名を検索キーにしません。INSERT UNIONのSELECT表と移送パターンは同じ採番計画を共有し、テーブル利用情報は表示別名へ変換せず物理IDのまま保持します。
