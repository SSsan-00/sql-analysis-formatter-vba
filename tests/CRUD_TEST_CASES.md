...
関数や演算式に含まれるCASEは、外側の式を`SUM(CASE結果)`のように残して右側へ分岐を表示します。
列エイリアスのないCASEを取得項目として直接返す場合は、Q列へ`CASE結果`、AF列以降へ分岐を複数行で表示します。
WHEN条件のANDとORは複数行へ分け、THENまたはELSEのネストCASEは一段右へ表示します。
...
SELECT INTOは複雑度にかかわらず、サブクエリ表、`＜DB入出力項目定義＞`、`＜データ移送表＞`の順に表示します。
...
| SEL-065 | FROMなしUPDATEのEXISTS検索条件 | 暫定確定・レビュー待ち |
| SEL-066 | UPDATE検索条件のスカラーサブクエリ | 暫定確定・レビュー待ち |
| SEL-067 | UPDATE検索条件のINサブクエリ | 暫定確定・レビュー待ち |
| SEL-068 | SUM内のCASE | 暫定確定・レビュー待ち |
| SEL-069 | ELSE内のネストCASE | 暫定確定・レビュー待ち |
| SEL-070 | 複合WHEN条件のCASE | 暫定確定・レビュー待ち |
| SEL-071 | UPDATE SET内のCASE | 暫定確定・レビュー待ち |
| SEL-072 | INSERT VALUES内のCASE | 暫定確定・レビュー待ち |
| SEL-073 | TOP内のCASE | 暫定確定・レビュー待ち |
| SEL-074 | OFFSET内のCASE | 暫定確定・レビュー待ち |
| SEL-075 | 取得結果を直接返すCASE | 暫定確定・レビュー待ち |

SEL-060、SEL-062からSEL-075はA5M2 2.21.2の`Ctrl+Q`で実整形した入力を使用しています。
